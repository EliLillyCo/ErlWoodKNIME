...
public class GetCompoundURIExec extends AbstractExec {
	...
	protected void initialiseSettings(BufferedDataTable input, OpenPhactsSettings openPhactsSettings) {
		...
		GetCompoundURISettings s = openPhactsSettings.getCompoundURISettings();
				
		s1 = s.getSmilesSettings();
		matchType = eMatchType.valueOf(s.getMatchTypeSetting().getValue());
		iSmilesIndex = s1.isUseColumn() ? input.getSpec().findColumnIndex(s1.getColumnName()) : -1;
	...
	protected List<RESTParameter<?>> getParams(DataRow row) {
		...
		String smiles = iSmilesIndex != -1 ? row.getCell(iSmilesIndex).toString() : s1.getValue();
		
		retValue.add(new RESTParameter<String>("searchOptions.Molecule", smiles));
		retValue.add(new RESTParameter<Integer>("searchOptions.MatchType", matchType.ordinal()));
