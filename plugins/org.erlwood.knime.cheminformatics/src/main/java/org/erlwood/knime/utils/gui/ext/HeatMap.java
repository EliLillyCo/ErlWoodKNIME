 ...
 * ------------------------------------------------------------------------
*/
package org.erlwood.knime.utils.gui.ext;
...
public final class HeatMap extends JPanel {
	...
	public HeatMap(DistanceVectorDataValue[] data, Color[] colors) {
		super();

		updateGradient2(colors);
		updateData2(data);

		this.setPreferredSize(new Dimension(60 + data2.length,
				60 + data2.length));
		this.setDoubleBuffered(true);
	...
	public void updateGradient2(Color[] colors) {
		this.colors = (Color[]) colors.clone();
	...
	private void updateDataColors() {
		...
		for (int x = 0; x < data.length; x++) {
			for (int y = 0; y < data[0].length; y++) {
				double norm = (data[x][y] - smallest) / range; 
				// 0 < norm < 1
	...
	private void updateDataColors2() {
		// We need to find the range of the data values,
		// in order to assign proper colors.
		double dLargest = Double.MIN_VALUE;
		double dSmallest = Double.MAX_VALUE;
		for (int x = 0; x < data2.length - 1; x++) {
			for (int y = x; y < data2.length; y++) {
				double v = data2[x].getDistance(data2[y]);
				dLargest = Math.max(v, dLargest);
				dSmallest = Math.min(v, dSmallest);
			}
		}
		double range = dLargest - dSmallest;

		// dataColorIndices is the same size as the data array
		// It stores an int index into the color array
		dataColorIndices = new int[data2.length][data2.length];

		// assign a Color to each data point
		for (int x = 0; x < data2.length; x++) {
			double v = data2[x].getDistance(data2[x]);
			// 0 < norm < 1
			double norm = (v - dSmallest) / range; 
			int colorIndex = (int) Math.floor(norm * (colors.length - 1));
			dataColorIndices[x][x] = colorIndex;
			for (int y = x + 1; y < data2.length; y++) {
				v = data2[x].getDistance(data2[y]);
				// 0 < norm < 1
				norm = (v - dSmallest) / range; 
				colorIndex = (int) Math.floor(norm * (colors.length - 1));
				dataColorIndices[x][y] = colorIndex;
				dataColorIndices[y][x] = colorIndex;
			}
		}
	}
	...
	public static double[][] generateSinCosData(int dimension) {
		...
		for (int x = 0; x < dimension; x++) {
			for (int y = 0; y < dimension; y++) {
				// 0 < sX < 2 * Pi
				sX = 2 * Math.PI * (x / (double) dimension);
				// 0 < sY < 2 * Pi
				sY = 2 * Math.PI * (y / (double) dimension); 
	...
	public static double[][] generatePyramidData(int dimension) {
		...
		for (int x = 0; x < dimension; x++) {
			for (int y = 0; y < dimension; y++) {
				// 0 < sX < 6
				sX = 6 * (x / (double) dimension); 
				// 0 < sY < 6
				sY = 6 * (y / (double) dimension); 
				// -3 < sX < 3
				sX = sX - 3; 
				// -3 < sY < 3
				sY = sY - 3; 
	...
	public void updateData2(DistanceVectorDataValue[] data) {
		this.data2 = data;
		updateDataColors2();
		drawData2();
		repaint();
	...
	private void drawData2() {
		bufferedImage = new BufferedImage(data2.length, data2.length,
				BufferedImage.TYPE_INT_ARGB);
		bufferedGraphics = bufferedImage.createGraphics();
