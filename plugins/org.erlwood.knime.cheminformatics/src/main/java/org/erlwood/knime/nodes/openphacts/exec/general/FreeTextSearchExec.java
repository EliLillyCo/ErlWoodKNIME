 ...
 * ------------------------------------------------------------------------
*/
package org.erlwood.knime.nodes.openphacts.exec.general;
...
public class FreeTextSearchExec extends AbstractExec {
	private AbstractColumnAndValueSettings s1 = null;
	
	private int inFreeTextIndex;
	...
	protected void initialiseSettings(BufferedDataTable input, OpenPhactsSettings openPhactsSettings) {
		...
		FreeTextSearchSettings s = openPhactsSettings.getFreeTextSearchSettings();
				
		s1 = s.getFreeTextSettings();
		
		inFreeTextIndex = s1.isUseColumn() ? input.getSpec().findColumnIndex(s1.getColumnName()) : -1;
	...
	protected List<RESTParameter<?>> getParams(DataRow row) {
		...
		String text = inFreeTextIndex != -1 ? row.getCell(inFreeTextIndex).toString() : s1.getValue();
		
		retValue.add(new RESTParameter<String>("query", text));
