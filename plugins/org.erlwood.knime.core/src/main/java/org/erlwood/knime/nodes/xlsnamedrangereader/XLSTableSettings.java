...
public class XLSTableSettings {
    ...
    private String m_firstNamedRange;
    ...
    private static void setMinMaxColumnAndRow(final XLSUserSettings settings,
        ...
        if (sheetName == null || sheetName.isEmpty() || sheetName.equals(XLSNamedRangeReaderNodeDialogPane.FIRST_SHEET)) {
            sheetName = XLSTable.getFirstSheetNameWithData(wb);
        }
        
        String NamedRange = settings.getNamedRange();
        if (NamedRange == null) {
            NamedRange = XLSTable.getFirstNamedRange(wb);
        }
        ...
        for (int i = 0; i < colHdrs.length; i++) {
            ...
            if (colHdrs[i] == null) {
            	...
            	if (settings.getReadNamedRange()) {
            		colName = settings.getNamedRange() + "_" + i;
            	}
            	else {
            		colName = "Col" + i;
            	}
    ...
    private static String[] readColumnHeaders(final XLSUserSettings settings,
        ...
        if ((settings.getReadNamedRange()) && !settings.getHasColHeaders()) {
        	result[0] = settings.getNamedRange().toString();
        	return result;
        }
    ...
    public String getNamedRange(final Workbook wb) {
        String NamedRange = m_userSettings.getNamedRange();
        if (NamedRange == null) {
            if (m_firstNamedRange == null) {
                try {
                    m_firstNamedRange =
                            XLSTable.getFirstNamedRange(wb);
                } catch (Exception e) {
                    m_firstNamedRange= "<error>";
                }
            }
            NamedRange = m_firstNamedRange;
        }
        return NamedRange;
    }
	...
	public String getReadNamedRange() {
		return m_userSettings.getNamedRange();
