 ...
 */
class XLSTable implements DataTable {
    ...
    XLSTable(final XLSUserSettings settings, final Workbook wb) throws InvalidSettingsException,
        ...
        m_workbook = wb;
		int firstRowNamedRange;
		int lastRowNamedRange;
		int firstColNamedRange;
		int lastColNamedRange;
        if (settings == null) {
        ...
        if (settings.getReadNamedRange()) {
        		...
        		firstColNamedRange = crefs[0].getCol();
        		lastColNamedRange = crefs[crefs.length - 1].getCol();
        		settings.setFirstColumn(firstColNamedRange);
        		settings.setLastColumn(lastColNamedRange);
        		settings.setFirstRow(firstRowNamedRange);
        		settings.setLastRow(lastRowNamedRange);
        }
    ...
    public static ArrayList<String> getNamedRanges(final Workbook wb, final String sheet) {
    	String wbsheet;
        ArrayList<String> names = new ArrayList<String>();
