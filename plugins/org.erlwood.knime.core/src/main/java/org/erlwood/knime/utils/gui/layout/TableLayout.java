...
public final class TableLayout implements java.awt.LayoutManager2,
	...
	public TableLayoutConstraints getConstraints(Component component) {
		...
		while (iterator.hasNext()) {
			...
			if (entry.component == component) {
				return new TableLayoutConstraints(entry.getCol1(), entry.getRow1(),
						entry.getCol2(), entry.getRow2(), entry.gethAlign(), entry.getvAlign());
			}
		}
	...
	public void drawGrid(Container container, Graphics g) {
		...
		for (int row = 0; row < rowSize.length; row++) {
			...
			for (int column = 0; column < columnSize.length; column++) {
			    // Draw the cell as a transparent rectangle
	...
	public boolean overlapping() {
		...
		for (int knowUnique = 1; knowUnique < numEntry; knowUnique++) {
			for (int checking = knowUnique - 1; checking >= 0; checking--) {
				if (((entry[checking].getCol1() >= entry[knowUnique].getCol1())
						&& (entry[checking].getCol1() <= entry[knowUnique].getCol2())
						&& (entry[checking].getRow1() >= entry[knowUnique].getRow1()) && (entry[checking].getRow1() <= entry[knowUnique].getRow2()))
						|| ((entry[checking].getCol2() >= entry[knowUnique].getCol1())
								&& (entry[checking].getCol2() <= entry[knowUnique].getCol2())
								&& (entry[checking].getRow2() >= entry[knowUnique].getRow1()) && (entry[checking].getRow2() <= entry[knowUnique].getRow2()))) {
					overlapping = true;
					break;
				}
			}
		...
		for (counter = 0; counter < numRow; counter++) {
			...
			if ((rowSpec[counter] == PREFERRED)
				...
				while (iterator.hasNext()) {
					...
					if ((entry.getRow1() == counter) && (entry.getRow2() == counter)) {
	...
	public void layoutContainer(Container container) {
		// Coordinates of the currnet component in pixels
		int x, y;
		// Width and height of the current component in pixels
		int w, h; 
		...
		for (int counter = 0; counter < component.length; counter++) {
			try {
				...
				if (entry.singleCell) {
					...
					if ((entry.gethAlign() == FULL) || (cellWidth < preferredWidth)) {
					...
					w = columnOffset[entry.getCol2() + 1] - columnOffset[entry.getCol1()];
	...
	public Dimension preferredLayoutSize(Container container) {
		// Preferred size of current component
		Dimension size; 
		// Preferred width of scalled components
		int scaledWidth = 0; 
		// Preferred height of scalled components
		int scaledHeight = 0; 
		...
		while (iterator.hasNext()) {
			...
			if ((entry.getCol1() < 0) || (entry.getCol1() >= columnSpec.length)
					|| (entry.getCol2() >= columnSpec.length) || (entry.getRow1() < 0)
					|| (entry.getRow1() >= rowSpec.length)
					|| (entry.getRow2() >= rowSpec.length)) {
	...
	public Dimension minimumLayoutSize(Container container) {
		// Minimum size of current component
		Dimension size; 
		// Minimum width of scalled components
		int scaledWidth = 0; 
		// Minimum height of scalled components
		int scaledHeight = 0; 
	...
	protected class Entry extends TableLayoutConstraints {
		...
		public Entry(Component component, TableLayoutConstraints constraint) {
			super(constraint.getCol1(), constraint.getRow1(), constraint.getCol2(),
					constraint.getRow2(), constraint.gethAlign(), constraint.getvAlign());
