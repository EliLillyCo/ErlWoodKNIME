 ...
 */
public class XLSNamedRangeReaderNodeModel extends NodeModel {

    private static final NodeLogger LOGGER = NodeLogger
            .getLogger(XLSNamedRangeReaderNodeModel.class);
     ...
     *
     */
    public XLSNamedRangeReaderNodeModel() {
        super(0, 1);
    }

    XLSNamedRangeReaderNodeModel(final NodeCreationContext context) {
        this();
    ...
    protected void loadValidatedSettingsFrom(final NodeSettingsRO settings)
        ...
        try {
            String settingsID = settings.getString(
                    XLSNamedRangeReaderNodeDialogPane.XLS_CFG_ID_FOR_TABLESPEC);
            if (!m_settings.getID().equals(settingsID)) {
                throw new InvalidSettingsException("IDs don't match");
            }
            NodeSettingsRO dtsConfig =
                    settings.getNodeSettings(XLSNamedRangeReaderNodeDialogPane.XLS_CFG_TABLESPEC);
    ...
    protected void saveSettingsTo(final NodeSettingsWO settings) {
        if (m_settings != null) {
            ...
            if (m_dts != null) {
                settings.addString(XLSNamedRangeReaderNodeDialogPane.XLS_CFG_ID_FOR_TABLESPEC,
                        m_dtsSettingsID);
                m_dts.save(settings
                        .addConfig(XLSNamedRangeReaderNodeDialogPane.XLS_CFG_TABLESPEC));
            }
        }
    }
